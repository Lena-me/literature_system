...
const router = createRouter({
  ...
  routes: [
    ...
    { path: '/research', component: () => import('@/layouts/ResearchLayout.vue'), children: [
      { path: '', component: () => import('@/views/research/WorkbenchView.vue') },
...
router.beforeEach(async (to) => {
  ...
  if (to.path.startsWith('/admin') && auth.user?.role !== 'admin') return '/research'
  if (to.path === '/login' && auth.isAuthed) return auth.user?.role === 'admin' ? '/admin' : '/research'
})
