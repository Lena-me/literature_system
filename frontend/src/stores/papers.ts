...
export const usePaperStore = defineStore('papers', {
  ...
  actions: {
    ...
    async open(paper: Paper) { this.current = paper; this.content = await papersApi.content(paper.id) },
    async upload(file: File, onProgress?: (n: number) => void) { const paper = await papersApi.upload(file, onProgress); await this.load(); return paper },
    async batchUpload(files: File[], onProgress?: (n: number) => void) { const result = await papersApi.batchUpload(files, onProgress); await this.load(); return result },
