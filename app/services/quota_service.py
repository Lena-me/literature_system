...
class QuotaService:
    async def _expire_stale_parse_tasks(self, user_id: int) -> None:
        """
        清理异常残留任务。

        使用独立短事务，避免与上传/重解析主事务交叉 commit 引发 users 行死锁。
        """
    ...
    async def check_upload(self, db: AsyncSession, user_id: int, file_size: int) -> None:
        await self._expire_stale_parse_tasks(user_id)
