...
def _model_audit_payload(obj: ModelConfig) -> dict:
    return {
        'model_id': obj.id,
...
def _model_audit_risk(obj: ModelConfig, *, deactivated: bool = False) -> int:
    if obj.model_type != 'llm':
        return 0
    if deactivated or not obj.is_active:
        return 2
    return 1
...
async def create_model_config(
    ...
    await audit_action(
        ...
        user_id=admin.id,
        module='admin',
        operation_type='create_model_config',
        content=_model_audit_payload(obj),
        request=request,
        risk=_model_audit_risk(obj),
    )
    await db.commit()
...
async def update_model_config(
    ...
    deactivated = obj.model_type == 'llm' and values.get('is_active') is False
    if 'config' in values:
    ...
    await audit_action(
        ...
        user_id=admin.id,
        module='admin',
        operation_type='update_model_config',
        content={'changes': values, **_model_audit_payload(obj)},
        request=request,
        risk=_model_audit_risk(obj, deactivated=deactivated),
    )
    await db.commit()
    invalidate_llm_runtime_cache()
...
async def create_model_config(
    ...
    if data.model_type == 'llm':
        ...
        scenario = default_scenario_for_type(data.model_type)
        is_primary = False
...
async def update_model_config(
    ...
    if obj.model_type == 'llm':
        ...
        obj.scenario = default_scenario_for_type(obj.model_type)
        obj.is_primary = False
