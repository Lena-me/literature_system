...
def _resolve_local_path(model_name: str) -> str:
    ...
    model_id = model_name.split('/')[-1]               # bge-reranker-large
    model_id_alt = model_id.replace('.', '___')        # bge-reranker-large (no dots in this name, but kept for consistency)
    full_alt = model_name.replace('/', '___').replace('.', '___')  # BAAI___bge-reranker-large
